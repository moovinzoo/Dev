...
class BabynameParser:
    ...
    def __init__(self, dirname, year):
        ...
        with open(os.path.join(dirname, "{}.html".format(year)), "r") as f:
            text = f.read()
        self.rank_to_names_tuples = [(record[0], record[1], record[2]) for record in re.findall(r'<td>(\d+)</td> <td>(\w+)</td> <td>(\w+)</td>', text)]
        f.close()
    ...
    def parse(self, parsing_lambda):
        ...
        # return BabyRecord(self.year, rank, name, gender, rank_change=None)

        return [parsing_lambda(tuple) for tuple in self.rank_to_names_tuples]
